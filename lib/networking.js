...
var session_url = "https://user.turna.live/api/yggdrasil/sessionserver/session/minecraft/profile/";
var textures_mojang_url = "https://textures.minecraft.net/texture/";
var textures_ygg_url = "https://user.turna.live/textures/";
    ...
    request.get(
      {
        url: url,
        headers: {
          "User-Agent": "Crafatar (+https://user.turna.live/crafatar)",
        },
...
exp.save_texture = function (rid, tex_hash, outpath, callback) {
  if (tex_hash) {
    var textureURLMojang = textures_mojang_url + tex_hash;
    var textureURLYgg = textures_ygg_url + tex_hash;
    exp.get_from(rid, textureURLMojang, function (img, response, err) {
      if (err) {
        callback(err, response, null);
      } else {
        if (img == null) {
          exp.get_from(rid, textureURLYgg, function (img, response, err) {
            if (err) {
              callback(err, response, null);
            } else {
